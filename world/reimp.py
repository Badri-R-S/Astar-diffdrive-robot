...
def visualize(path_gen): #Function to visualize the graph
    ...
    for idx,any in enumerate(heap):
        if(any.parent is not None):
            start_pos = (any.parent[0],any.parent[1])
            print(start_pos)
            end_pos = (any[0],any[1])
            pygame.draw.line(surface,VISITED_COLOR,start_pos,end_pos,2)
            pygame.draw.rect(surface,PIXEL_COLOR,(any[0],any[1],1,1))
    ...
    for idx,every in enumerate(path_gen):
        if(every.parent is not None):
            pygame.draw.line(surface,PATH_COLOR,(every.parent[0],every.parent[1]),(every[0],every[1]),2)
            pygame.draw.rect(surface,PIXEL_COLOR,(every[0],every[1],1,1))
        # pygame.display.flip()
...
def get_goalcoord_input(): #Function to get goal coordinates from user
    ...
    #     print("orientation should be in multiples of 30")
    #     return [flag]
    if(xg<0 or xg>=600 or yg<0 or yg>=200):
...
if __name__ == "__main__":
    ...
    goal = [goal_input[1],goal_input[2],0]
    node.parent = None
    node.cost_to_come = 0
